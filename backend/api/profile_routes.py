...
async def add_explicit_interest(user_id: str = Body(None), keyword: str = Body(...), weight: float = Body(1.0),
    ...
    user_profiles_col.update_one({"user_id": effective_user}, {"$set": profile}, upsert=True)
    return profile

@router.put("/profiles/explicit/bulk_update")
async def bulk_update_explicit_interests(user_id: str = Body(None), updates: list = Body(...),
                                         auth_user: str = Depends(get_user_id_from_auth)):
    effective_user = auth_user if auth_user and auth_user != "guest" else (user_id or "guest")
    profile = build_user_profile(effective_user)
...
async def remove_explicit_interest(user_id: str = Body(None), keyword: str = Body(...),
                                   auth_user: str = Depends(get_user_id_from_auth)):
    effective_user = auth_user if auth_user and auth_user != "guest" else (user_id or "guest")
    profile = build_user_profile(effective_user)
...
async def remove_implicit_interest(user_id: str = Body(None), keyword: str = Body(...),
                                   auth_user: str = Depends(get_user_id_from_auth)):
    effective_user = auth_user if auth_user and auth_user != "guest" else (user_id or "guest")
    if not keyword:
    ...
    if keyword.lower() not in [e.lower() for e in exclusions]:
        ...
        user_profiles_col.update_one({"user_id": effective_user}, {"$set": {"implicit_exclusions": exclusions}}, upsert=True)
