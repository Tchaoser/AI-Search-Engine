"""
Runner for background tasks (moved out of `services`).

This module contains the ProfileRebuildThread and start/stop helpers.
It largely mirrors the previous implementation found in
`backend/services/background_tasks.py` but lives in its own package so
background tasks can be discovered/managed separately from `services`.
"""
